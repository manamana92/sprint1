...
unsigned char SubByte(unsigned char uchByte) {
    ...
    /**  @var rgrguchSbox 2-D integer array as defined in FIPS 197*/
    unsigned char rgrguchSbox[16][16] = {
...
void KeyExpansion(int iNr, int iNk, unsigned char rguchKey[iNk * 4],
                  unsigned char rgrguchKeySchedule[NB * (iNr + 1)][4]) {
    if (iNr != 10 && iNr != 12 && iNr != 14 && iNk != 4 && iNk != 6 &&
        iNk != 8) {
        return;
    }
    ...
    iIterator = iNk;
    /**  @var uchRcon Integer for storing first value of RCON*/
    ...
    while (iIterator < NB * (iNr + 1)) {
        ...
        if (iIterator % iNk == 0) {
            ...
            SubWord(rguchTemp);
            uchRcon = 1;
            for (int iInIterator = 0; iInIterator < ((iIterator / iNk) - 1);
                 iInIterator++) {
                uchRcon = Mult(uchRcon, 2);
		/**uchRcon = (uchRcon<<1);
                if(uchRcon>0x00ff){
                    uchRcon = uchRcon ^ 0x011b;
                }*/
            }
            /**uchRcon = (pow(2,((i/iNk)-1)));*/
            rguchTemp[0] = rguchTemp[0] ^ uchRcon;

        } else if (iNk > 6 && iIterator % iNk == 4) {
            SubWord(rguchTemp);
        }
        rgrguchKeySchedule[iIterator][0] =
            rgrguchKeySchedule[iIterator - iNk][0] ^ rguchTemp[0];
        rgrguchKeySchedule[iIterator][1] =
            rgrguchKeySchedule[iIterator - iNk][1] ^ rguchTemp[1];
        rgrguchKeySchedule[iIterator][2] =
            rgrguchKeySchedule[iIterator - iNk][2] ^ rguchTemp[2];
        rgrguchKeySchedule[iIterator][3] =
            rgrguchKeySchedule[iIterator - iNk][3] ^ rguchTemp[3];
